...
(() => {
  ...
  const matchesFilter = (story) => state.filter === 'all' || storyStatuses(story).includes(state.filter);
  const matchesSelection = (story) => {
    if (!state.selection) return true;
    if (state.selection.kind === 'component') return story.sourcePath === state.selection.path;
    const directoryPath = story.directories.join('/');
    return directoryPath === state.selection.path || directoryPath.startsWith(state.selection.path + '/');
  };
  const visibleStories = () => report.stories.filter((story) => matchesSearch(story) && matchesFilter(story) && matchesSelection(story));
  ...
  function buildTree(stories) {
    ...
    for (const story of stories) {
      ...
      for (const directory of story.directories) {
        path.push(directory);
        if (!node.children.has(directory)) node.children.set(directory, { children: new Map(), components: new Map(), path: path.join('/') });
        node = node.children.get(directory);
      }
      if (!node.components.has(story.sourcePath)) node.components.set(story.sourcePath, { component: story.component, sourcePath: story.sourcePath, stories: [] });
  ...
  function createDirectory(node, name) {
    ...
    summary.setAttribute('aria-current', state.selection && state.selection.kind === 'directory' && state.selection.path === node.path ? 'true' : 'false');
    ...
    for (const [childName, child] of node.children) details.append(createDirectory(child, childName));
    for (const component of node.components.values()) details.append(createComponent(component));
    return details;
  }

  function countStories(node) {
    let total = [...node.components.values()].reduce((sum, component) => sum + component.stories.length, 0);
  ...
  function createComponent(component) {
    ...
    button.setAttribute('aria-pressed', state.selection && state.selection.kind === 'component' && state.selection.path === component.sourcePath ? 'true' : 'false');
  ...
  function renderTree() {
    ...
    const stories = report.stories.filter((story) => matchesSearch(story) && matchesFilter(story));
    const rootButton = document.createElement('button');
    rootButton.type = 'button';
    rootButton.className = 'tree-root';
    rootButton.setAttribute('aria-pressed', state.selection === null ? 'true' : 'false');
    ...
    for (const [name, node] of root.children) tree.append(createDirectory(node, name));
    for (const component of root.components.values()) tree.append(createComponent(component));
  }
  ...
  function updateTreeSelection() {
    for (const item of tree.querySelectorAll('[data-selection-kind]')) {
      const selected = state.selection === null
        ...
        : item.dataset.selectionKind === state.selection.kind && item.dataset.selectionPath === state.selection.path;
      if (item instanceof HTMLSummaryElement) item.setAttribute('aria-current', String(selected));
  ...
  function createPane(label, source, story, variant, detail) {
    ...
    if (!source) {
      ...
      placeholder.textContent = label === 'Before' ? 'No baseline image' : label === 'After' ? 'No current image' : 'Diff unavailable';
    ...
    button.setAttribute('aria-label', 'Open ' + label + ' image for ' + story.storyId + ' (' + variant.name + ')');
    ...
    placeholder.textContent = label === 'Diff' ? 'Diff unavailable' : 'Image unavailable';
  ...
  function createComparison(variant, story, detail) {
    if (state.view === 'diff') {
      ...
      comparison.append(createPane('Diff', variant.diff, story, variant, detail));
      return comparison;
    }
    ...
    const comparison = document.createElement('div');
    comparison.className = 'comparison';
    comparison.append(createPane('Before', variant.before, story, variant, detail));
    comparison.append(createPane('After', variant.after, story, variant, detail));
    return comparison;
  }
  ...
  function createStory(story) {
    ...
    const changedVariants = story.variants.filter((variant) => variant.status !== 'unchanged');
    for (const variant of changedVariants) variants.append(createVariant(variant, story, false));
    article.append(head, variants);
    const unchanged = story.variants.filter((variant) => variant.status === 'unchanged');
    if (unchanged.length > 0) {
      ...
      toggle.addEventListener('click', () => {
        ...
        toggle.textContent = (open ? 'Show ' : 'Hide ') + unchanged.length + ' unchanged variants';
        if (open) {
          variants.querySelectorAll('[data-unchanged="true"]').forEach((item) => item.remove());
        } else {
  ...
  for (const [status, label] of [['changed', 'Changed'], ['new', 'New'], ['deleted', 'Deleted'], ['passed', 'Passed']]) {
  ...
  for (const [filter, label, value] of [
    ...
    button.addEventListener('click', () => {
      state.filter = filter;
      for (const item of filters.querySelectorAll('.filter')) item.setAttribute('aria-pressed', String(item === button));
      render();
    });
  ...
  for (const [view, label] of [['pair', 'Before + After'], ['diff', 'Diff'], ['slider', 'Slider']]) {
    const button = document.createElement('button');
    button.type = 'button';
    ...
    button.addEventListener('click', () => {
      state.view = view;
      for (const item of modes.querySelectorAll('.mode')) item.setAttribute('aria-pressed', String(item === button));
      renderList();
    });
    modes.append(button);
  }
